    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere(['like', 'identity_number', $this->identity_number])
            ...
            if ($this->placementProcessStatus) {
                ...
                $query->andWhere(['in', 'employee.id', LatestPlacementPlan::find()->select('employee_id')->where(['response_type' => $processStatus])]);
            }
        
            if ($this->placementClientName) {
                $query->andWhere(['in', 'employee.id', LatestPlacementPlan::find()->select('employee_id')->where(['client_id' => $this->placementClientName])]);
            }
