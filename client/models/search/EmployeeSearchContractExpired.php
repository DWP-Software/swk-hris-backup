 ...
 */
class EmployeeSearchContractExpired extends Employee
{
    ...
    public function rules()
    {
        return [
            ...
            [['placementProcessStatus', 'placementClientName', 'latestContract'], 'safe'],
    ...
    public function search($params)
    {
        ...
        $query->joinWith(['latestContractPlacement'])->andWhere(['client_id' => Yii::$app->user->identity->clientUser->client_id]);
        ...
        $query->andFilterWhere(['like', 'identity_number', $this->identity_number])
            ...
            if ($this->placementProcessStatus) {
                $processStatus = $this->placementProcessStatus == 3 ? null : $this->placementProcessStatus; 
                $query->andWhere(['in', 'employee.id', PlacementPlan::find()->select('employee_id')->where(['response_type' => $processStatus])]);
            }
        
            if ($this->placementClientName) {
                $query->andWhere(['in', 'employee.id', PlacementPlan::find()->select('employee_id')->where(['response_type' => $this->placementClientName])]);
            }
