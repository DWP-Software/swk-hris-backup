...
class PlacementPlanSearch extends PlacementPlan
{
    ...
    {
        $contracted_placements = ArrayHelper::getColumn(ContractPlacement::find()->select('placement_plan_id')->asArray()->all(), 'placement_plan_id');
        $query = PlacementPlan::find();
        $query->where(['client_id' => Yii::$app->user->identity->clientUser->client_id]);
        $query->andWhere(['not in', 'placement_plan.id', $contracted_placements]);
