...
$this->title = 'Karyawan <small>'.$subtitle.'</small>';
...
<div class="employee-index box-- box-primary-- box-body--">
    ...
    <?php Pjax::begin(); ?>    <?php 
        $exportColumns = [
            ...
            [
                'attribute' => 'family_number',
                'value' => function($model) {
                    return $model->family_number.' ';
                }
            ],
            'mother_name',
            'nationality',
        ...
        $gridColumns = [
            ...
            [
                ...
                'buttons' => [
                    ...
                    'placement-create' => function ($url, $model) {
                        ...
                        if ($pending_placement === null && !$model->activeContract)
                        return Html::a('', $url, [
                            'title' => 'assign', 
            ...
            [
                ...
                'value'     => function($model) {
                    ...
                    . '<br>NRP : ' . $model->registration_number;
                },
            ],
            [
                'attribute'           => 'latestContract',
                'label'               => 'Kontrak',
                'format'              => 'html',
                'value'               => function($model) {
                    return $model->latestContract->contract_started_at . ' - ' . $model->latestContract->contract_ended_at
                    . '<br>' . ($model->latestContract ? $model->latestContract->contract->employeeType->name : '') 
                    .'<br><span class="text-muted">' . ($model->latestContract ? $model->latestContract->contract->contractPhaseText : '') . '</span>';
                },
            ...
            [
                ...
                'format'              => 'html',
                'value'               => function($model) {
                    return ($model->latestContract->contract->contractPlacements ? $model->latestContract->contract->contractPlacements[0]->client->name . '<br><span class="text-muted">'.$model->latestContract->contract->contractPlacements[0]->position.'</span>' : '');
                },
                'filterType'          => GridView::FILTER_SELECT2,
                'filter'              => ArrayHelper::map(Client::find()->orderBy('name')->asArray()->all(), 'id', 'name'),
                'filterInputOptions'  => ['placeholder' => ''],
            ...
            [
                ...
                'format'    => 'raw',
                'value'     => function($model) {
                    return Html::a($model->user->email, ['/user-client/view', 'id' => $model->user->id])
                    . '<br> Telp: ' . $model->phone;
                },
                'filterType'          => GridView::FILTER_SELECT2,
    ...
    <?= GridView::widget([
        ...
        'toolbar'        => [
            ...
            '{toggleData}',
            Html::a('<i class="fa fa-file-excel-o"></i> &nbsp;' . 'Export', ['export', 'contract_status' => $contract_status], ['data-pjax'=>0, 'class'=>'btn btn-default']),
            // $exportMenu,
