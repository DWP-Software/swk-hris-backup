...
<style>
    ...
    <div class="" style="display: inline-block; vertical-align: middle">
        <input type="text" name="params[year]" class="form-control" placeholder="tahun..." value="<?= $params['year'] ?>" style='display:inline' />
    </div>
    ...
    <?= Select2::widget([
        ...
        'options' => ['placeholder' => 'semua mitra'],
    ...
    <?= Select2::widget([
        ...
        'data' => ArrayHelper::map(Employee::find()->all(), 'id', 'shortText'),
    ...
    <?= Html::a('<i class="fa fa-file-excel-o"></i> ' . Yii::t('app', 'Export'), ['spreadsheet', 'params' => ['client_id' => $params['client_id'], 'month' => $params['month'], 'year' => $params['year']]], [
        'class' => 'btn btn-default',
        // 'style' => 'border-top-left-radius:0; border-bottom-left-radius:0; margin-left:-1px',
    ]) ?>
    ...
    <div class="printable">
        ...
        <?php } else { ?>
            ...
            <table class="table table-bordered table-condensed" style="margin-bottom:0px">
                ...
                <?php 
                    $query = Employee::find()->joinWith(['contracts.contractPlacements'])->where(['not in', 'employee.id', $presenceIds]);
                    if ($employee_id) $query->andWhere(['employee.id' => $employee_id]);
                    if ($client_id) $query->andWhere(['contract_placement.client_id' => $client_id]);
                    $absences = $query->groupBy('employee.id')->asArray()->all();
                    foreach ($absences as $model) {
                        // $presences = ArrayHelper::index($model['presences'], 'date');
                ?>
                    <tr>
                        <td style="width:1px;white-space:nowrap;"><?= ++$i; ?></td>
