 ...
 */
class EmployeePreController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    public function actionContractDownload($contract_id) 
    {
        if (($contract = Contract::findOne($contract_id)) !== null) {
            if ($contract->file) {
                ...
                $extension = end($array);
                $filename  = $contract->placement->employee->name . '.' . $extension;
    ...
    public function actionExport()
    {
        ...
        $columns_salary = ['V', 'W', 'X', 'Y', 'Z', 'AA', 'AB', 'AC', 'AD', 'AE', 'AF', 'AG'];
        $j = 0;
        foreach (ContractSalary::types() as $type_key => $type_value) {
            foreach (ContractSalary::permanentTypes($type_key) as $subtype_key => $subtype_value) {
        ...
        $sheet->setCellValue('AH3', 'Bank');
        $sheet->setCellValue('AI3', 'No. Rekening');
        $sheet->setCellValue('AJ3', 'NPWP');

        $sheet->setCellValue('AK3', 'Keluarga');
        $columns_family = ['AK', 'AL', 'AM'];
        $j = 0;
        foreach (EmployeeFamily::positions() as $position_key => $position_value) {
            $sheet->setCellValue($columns_family[$j++].'4', $position_value);
        }

        $sheet->setCellValue('AN3', 'Kontak Darurat');

        $sheet->setCellValue('AO3', 'Kontrak');
        ...
        $sheet->getStyle('A3:AO4')->applyFromArray($styleHeader);
        ...
        foreach ($employees as $employee) {
            ...
            $sheet->setCellValue('P'.$currentRow, $employee->religion);
            ...
            $sheet->setCellValue('AH'.$currentRow, $employee->bank_name);
            $sheet->setCellValue('AI'.$currentRow, $employee->bank_account);
            $sheet->setCellValue('AJ'.$currentRow, $employee->tax_number);
            ...
            if ($emergencies) {
                ...
                $sheet->setCellValue('AN'.$currentRow, $emergencyText);
            }

            $contracts = EmployeeEmergency::find()->where(['employee_id' => $employee->id])->all();
            if ($contracts) {
                $array = [];
                ...
                $sheet->setCellValue('AO'.$currentRow, $contractText);
            }
        }
    ...
    public function actionDeleteAll()
    {
        ...
        Employee::deleteAll(['in', 'employee.id', $uncontracted_employees]);
        return $this->redirect(['index']);
    }
}
